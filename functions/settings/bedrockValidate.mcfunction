...
execute @s[type=player,tag=op,scores={bedrock=..0,gametestapi=1..}] ~~~ tellraw @a[tag=op] {"rawtext":[{"text":"§r§j[§uIsolate§j]§r "},{"selector":"@s"},{"text":" has enabled §aBedrockValidate."}]}

# deny
execute @s[type=player,tag=op,scores={bedrock=1..,gametestapi=1..}] ~~~ scoreboard players set scythe:config bedrock 0
execute @s[type=player,tag=op,scores={bedrock=1..,gametestapi=1..}] ~~~ tellraw @a[tag=op] {"rawtext":[{"text":"§r§j[§uIsolate§j]§r "},{"selector":"@s"},{"text":" has disabled §4BedrockValidate."}]}
