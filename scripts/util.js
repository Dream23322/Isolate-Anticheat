 ...
 */
export function flag(player, check, checkType, hackType, debugName, debug, shouldTP, cancelObject, slot) {
    ...
    if(shouldTP === true && check !== "Killaura" && check !== "Scaffold") {
    ...
    } else if(check === "Killaura") {
        player.teleport(player.location.x, player.location.y, player.location.z + 0.5, player.dimension, 10, 10, 10, false)
    } else if (check === "Scaffold") {
        player.teleport(player.location.x, player.location.y, player.location.z - 0.5, player.dimension, 10, 360, 10, true)
    }
    ...
    if(debug) {
        ...
        const banLength = checkData.banLength;
        ...
        if (punishment == "mute") {
            ...
            player.sendMessage(`§r§j[§uIsolate§j]§r You have been muted by §uIsolate Anticheat§r for Spamming/Unfair-Advantage.`);
    
            // remove chat ability
            player.runCommandAsync("ability @s mute true");

            player.runCommandAsync(`tellraw @a[tag=notify] {"rawtext":[{"text":"§r§j[§uIsolate§j]§r ${player.name} has been automatically muted by §uIsolate Anticheat§r for Unfair Advantage. Check: ${check}/${checkType}"}]}`);
            
        } else if (punishment == "kick" && player.getScore(player, `${check}vl`) >= checkData.minVlbeforePunishment) {
            try {
                player.runCommandAsync(`tellraw @a[tag=notify] {"rawtext":[{"text":"§r§j[§uIsolate§j]§r ${player.name} has been automatically kicked by §uIsolate Anticheat§r for Unfair Advantage. Check: ${check}/${checkType}"}]}`);
                player.runCommandAsync(`kick "${player.name}" §r§j[§uIsolate§j]§r You have been kicked for using an unfair advantage.`);
            } catch (error) {// incase /kick fails, we despawn them from the world
                player.triggerEvent("scythe:kick");
            }
        } else if (punishment == "ban" && player.getScore(player, checkData) >= checkData.minVlbeforePunishment) {
            if(getScore(player, "autoban") >= 1) {
                const punishmentLength = checkData.punishmentLength?.toLowerCase();
                
                player.runCommandAsync(`tellraw @a[tag=notify] {"rawtext":[{"text":"§r§j[§uIsolate§j]§r ${player.name} has been banned by §uIsolate Anticheat§r for Unfair Advantage. Check: ${check}/${checkType}"}]}`);
                ...
                player.addTag(`reason:§uIsolate §cAnticheat detected the use of an §6Unfair Advantage`);
