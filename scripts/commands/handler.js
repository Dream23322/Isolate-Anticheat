...
export function commandHandler(msg) {
    ...
    try {
        ...
        if(commandData.requiredTags.length >= 1 && commandData.requiredTags.some(tag => !player.hasTag(tag))) {
            player.sendMessage("§r§j[§uIsolate§j]§r You need to be Scythe-Opped to use this command. To gain isolate-op please run: /function op");
            return;
        }
        ...
        if(args.length < commands[commandName].minArgCount) {
            player.sendMessage(`§r§j[§uIsolate§j]§r Invalid command usage.\n${prefix}${commandName} ${commands[commandName].usage}`);
            return;
        }
        ...
        player.sendMessage(`§r§j[§uIsolate§j]§r There was an error while trying to run this command. Please forward this message to 4urxra#0.\n-------------------------\nCommand: ${message}\n${error}\n${error.stack || "\n"}-------------------------`);
    }
}
    ...
    system.run(async () => {
        try {
           await commands[commandName].execute(msg, args, commandName);
            ...
            console.error(`${new Date().toISOString()} | ${error} ${error.stack}`);
            message.sender.sendMessage(`§r§j[§uIsolate§j]§r There was an error while trying to run this command. Please forward this message to https://discord.gg/9m9TbgJ973.\n-------------------------\nCommand: ${message.message}\n${error}\n${error.stack || "\n"}-------------------------`);
        }
    });
}