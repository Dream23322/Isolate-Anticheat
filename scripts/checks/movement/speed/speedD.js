...
export function speed_d(player) {
    ...
    if(config.modules.speedD.enabled) {
        if(data.get(player.name)) {
            const currentY = isomath.abs(player.getVelocity().y);
            const dY1 = isomath.abs(data.get(player.name)?.one);
            const dY2 = isomath.abs(data.get(player.name)?.two);
            const dY3 = isomath.abs(data.get(player.name)?.three);
