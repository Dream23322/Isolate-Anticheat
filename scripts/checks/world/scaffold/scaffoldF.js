...
export function scaffold_f(player, block) {
    ...
    if(config.modules.scaffoldF.enabled) {
        const distance = fastSqrt(Math.pow(block.location.x - player.location.x, 2) + Math.pow(block.location.y - player.location.y, 2) + Math.pow(block.location.z - player.location.z, 2));
